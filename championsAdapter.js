...
function makePokemon(payload = {}) {
  ...
  const resolvedSpecies = canonicalPokemon(payload.species);
  ...
  if (!pokemon.species || !pokemon.species.baseStats) {
    throw new Error(`Unknown species: ${payload.species} (resolved as ${resolvedSpecies}). Add it to calculator/data/zh_aliases.json or BUILTIN_ALIASES.`);
  }
